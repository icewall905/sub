...
def run_web_ui():
        ...
        <div class="container">
            ...
            <form action="/upload" method="POST" enctype="multipart/form-data" id="uploadForm">
                ...
                <div class="upload-method-selector">
                    <label>
                        <input type="radio" name="upload-method" value="upload" checked> Upload File
                    </label>
                    <label>
                        <input type="radio" name="upload-method" value="host"> Browse Host Files
                    </label>
                </div>
                
                <div id="upload-method-container">
                    <!-- File Upload Option -->
                    <div id="upload-option" class="upload-options">
                        <input type="file" id="srtfile" name="srtfile" accept=".srt" required>
                    </div>
                    
                    <!-- Host File Browser Option -->
                    <div id="host-file-option" class="upload-options" style="display: none;">
                        <div class="host-file-browser">
                            <div class="browser-header">
                                <span id="current-path">Home Directory</span>
                            </div>
                            <div class="browser-content">
                                <ul id="host-directory-list" class="directory-list">
                                    <li class="loading">Loading files...</li>
                                </ul>
                            </div>
                        </div>
                        <div id="selected-file-info" class="selected-file-info" style="display: none;">
                            <strong>Selected:</strong> <span id="selected-file-path"></span>
    ...
    def browse_files():
        """API endpoint to browse files and directories on the host system"""
        path = request.args.get('path', os.path.expanduser('~'))
        
        # Security check - don't allow browsing outside of user's home directory
        ...
        try:
            ...
            parent_path = os.path.dirname(path) if path != os.path.expanduser('~') else None
            ...
            append_log(f"[ERROR] Error browsing files: {str(e)}")
            return jsonify({"error": f"Error browsing files: {str(e)}"})

    @app.route('/upload_host_file', methods=["POST"])
    def upload_host_file():
        """Handle uploading a file from the host system"""
        try:
            host_file_path = request.form.get('host_file_path')
            
            if not host_file_path or not os.path.exists(host_file_path):
                return jsonify({"error": "Invalid file path or file doesn't exist"}), 400
            
            if not host_file_path.lower().endswith('.srt'):
                return jsonify({"error": "Only SRT files are supported"}), 400
            
            # Same logic as the regular upload route, but using a local file
            source_lang = request.form.get('source_language', 'en')
            target_lang = request.form.get('target_language', 'da')

            # Create output directory
            output_dir = os.path.join(os.getcwd(), 'files', time.strftime('%Y%m%d-%H%M%S'))
            os.makedirs(output_dir, exist_ok=True)
            
            # Copy the file to our work directory
            file_name = os.path.basename(host_file_path)
            srt_file_path = os.path.join(output_dir, file_name)
            shutil.copy(host_file_path, srt_file_path)
            
            # Get the config and translate
            cfg = load_config()
            
            # Update config with form values
            if not cfg.has_section('general'):
                cfg.add_section('general')
            cfg.set('general', 'source_language', source_lang)
            cfg.set('general', 'target_language', target_lang)
            
            translated_file = translate_srt(srt_file_path, None, cfg)
            
            # Same as the regular upload route's redirect
            return redirect(f'/download/{os.path.basename(output_dir)}/{os.path.basename(translated_file)}')
            
        except Exception as e:
            append_log(f"[ERROR] Error processing host file: {str(e)}")
